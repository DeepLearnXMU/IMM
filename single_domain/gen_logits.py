...
from reader.gen_logits_squad import DataProcessor, write_predictions
...
data_g.add_arg("adv_text_path",             str,   None,  "Adversarial training dataset")
...
def create_model(pyreader_name, bert_config, is_training=False):
    if is_training:
        ...
        #     name=pyreader_name,
        #     use_double_buffer=True)
        # (src_ids, pos_ids, sent_ids, input_mask) = fluid.layers.read_file(pyreader)

        src_ids = fluid.layers.data(shape=[args.batch_size, args.max_seq_len, 1], dtype='int64', name="input_ids", append_batch_size=False, lod_level=0)
        pos_ids = fluid.layers.data(shape=[args.batch_size, args.max_seq_len, 1], dtype='int64', name="position_ids", append_batch_size=False, lod_level=0)
        sent_ids = fluid.layers.data(shape=[args.batch_size, args.max_seq_len, 1], dtype='int64', name="sentence_ids", append_batch_size=False, lod_level=0)
        input_mask = fluid.layers.data(shape=[args.batch_size, args.max_seq_len, 1], dtype='float32', name="input_mask", append_batch_size=False, lod_level=0)
    ...
    if is_training:
        ...
        return total_loss, num_seqs
    else:
        return start_logits, end_logits, num_seqs, input_mask
...
def predict(test_exe, test_program, test_data_generator, fetch_list, processor):
    ...
    # logit_file = open("logits_file", 'w')

    qas_id2logits = {}

    while True:
        try:
            all_inputs = test_data_generator.next()

            feed_dict = {
                    "input_ids": all_inputs[0],
                    "position_ids": all_inputs[1],
                    "sentence_ids": all_inputs[2],
                    "input_mask": all_inputs[3],
                }
            qas_id = all_inputs[4]
            #print(qas_id)
            # print(feed_dict)
            np_start_logits, np_end_logits, np_num_seqs, input_mask = test_exe.run(
                feed=feed_dict, fetch_list=fetch_list, program=test_program)

            write_start_logits = np_start_logits
            write_end_logits = np_end_logits
            for i in range(write_end_logits.shape[0]):
                qas_id2logits[str(qas_id[i])] = [write_start_logits[i][:int(input_mask[i].sum())], write_end_logits[i][:int(input_mask[i].sum())]]
                # logit_file.write(str(qas_id[i]))
                # logit_file.write('\n')
                # logit_file.write(' '.join([str(x) for x in write_start_logits[i][:int(input_mask[i].sum())]]))
                # logit_file.write('\n')
                # logit_file.write(' '.join([str(x) for x in write_end_logits[i][:int(input_mask[i].sum())]]))
                # logit_file.write('\n')
                # logit_file.write('\n')
            continue
            ...
            logit_file = open("logits_file", 'w')
            for qas_id in qas_id2logits.keys():
                logit_file.write(qas_id)
                logit_file.write('\n')
                logit_file.write(' '.join([str(x) for x in qas_id2logits[qas_id][0]]))
                logit_file.write('\n')
                logit_file.write(' '.join([str(x) for x in qas_id2logits[qas_id][1]]))
                logit_file.write('\n')
                logit_file.write('\n')
            logit_file.close()
            break
    ...
    adv_f1 = write_predictions(processor.predict_examples, features, all_results,
    ...
    if args.do_predict:
        ...
        with fluid.program_guard(test_prog, startup_prog):
            with fluid.unique_name.guard():
                ...
                fluid.memory_optimize(test_prog, skip_opt_set=[
            ...
            try:
                ...
                if steps % args.skip_steps == 0:
                    if warmup_steps <= 0:
                        ...
                        fetch_list = [
                            loss.name, scheduled_lr.name, num_seqs.name, input_mask.name
                        ]
                else:
                    fetch_list = []
